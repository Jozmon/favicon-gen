...
##inkscape: use drawing area in exports 1 or 0 to enable/disable
USE_AREA_DRAWING=0


##enables or disables area drawing
if [ $USE_AREA_DRAWING -eq 1 ]
	then
		AREA_DRAWING="--export-area-drawing"
elif [ $USE_AREA_DRAWING -eq 0 ]
	then
		AREA_DRAWING=" "
fi
...
for RES in 16 32 57 76 96 128 152 167 180 192 196 228
    do
        inkscape $AREA_DRAWING \
        --export-png=./$NAME-favicons/favicon-$RES.png \
        --export-width=$RES \
        --export-height=$RES "$SVG_IMAGE" 2> /dev/null
